          ...
          <div key={i} className="bg-zinc-900/80 border border-zinc-800 rounded-[2.5rem] p-8 shadow-2xl relative">
            ...
            <div className="flex flex-wrap gap-1.5 mb-6">
               {w.numbers.split(', ').map((n, idx) => (
                 <span key={idx} className="w-8 h-8 flex items-center justify-center rounded-full bg-zinc-800 text-[11px] font-black text-white border border-zinc-700">{n}</span>
               ))}
               <span className="text-zinc-700 mx-1">|</span>
               {w.spirit.split(', ').map((n, idx) => (
                 <span key={idx} className="w-8 h-8 flex items-center justify-center rounded-full bg-red-900/30 text-[11px] font-black text-red-500 border border-red-900/50">{n}</span>
               ))}
            </div>
...
export default function MarpoLottoPage() {
  ...
  // 1. [수정] 티켓 데이터 호출 함수 (외부 통합 및 useCallback)
  const fetchMyTickets = useCallback(async (userId: string) => {
    ...
    } catch (error) { console.error("Ticket fetch error"); }
  }, []);
  ...
  const fetchOracleSettings = useCallback(async () => {
    ...
    } catch (e) { console.error("Oracle Sync Fail"); }
  }, []);

  // 3. 초기화 로직 (함수 중복 정의 제거)
  useEffect(() => {
  ...
  const handleClaimPrize = async (ticket: any) => {
    if (!confirm(`${ticket.amount} Pi 수령을 신청하시겠습니까?`)) return;
    try {
      const response = await fetch('/api/tickets/claim', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ticketId: ticket.id, userId: user?.username, drawDate: ticket.createdAt, prizeAmount: ticket.amount })
      });
      if (response.ok) { 
        alert("수령 신청 완료!"); 
    ...
    <div className="min-h-screen bg-black text-white flex flex-col items-center p-4 font-sans relative pb-40 text-center">
      ...
      <section className="w-full max-w-md bg-zinc-900 border border-yellow-500/20 p-8 rounded-[2.5rem] mb-12 shadow-2xl relative overflow-hidden">
          ...
          <div className="pt-6 border-t border-zinc-800/80 flex justify-between items-end px-1">
            ...
            <div className="text-right"><p className="text-xl font-black text-white tracking-tight lead-none">$ {peggedUsd.toLocaleString(undefined, {minimumFractionDigits: 3})} <span className="text-[10px] text-zinc-500 ml-1.5 font-bold uppercase">usd</span></p></div>
          </div>
      </section>
          ...
          {activeTickets.length > 0 ? (
            <div className="flex flex-col gap-4">
              {activeTickets.map((t, i) => (
                <div key={i} className="bg-zinc-900 border border-zinc-800 rounded-[2.5rem] p-8 text-center shadow-xl">
                  ...
                  {/* 카운트다운 박스 내 선택 번호 강제 노출 */}
                  <div className="flex flex-wrap gap-1.5 justify-center pt-4 border-t border-zinc-800/50">
                    {t.selectedNumbers?.main?.map((n:number, idx:number) => <span key={idx} className="w-7 h-7 flex items-center justify-center rounded-full bg-zinc-800 text-[10px] font-black text-white">{n}</span>)}
                    <span className="text-zinc-700 mx-1">|</span>
                    {t.selectedNumbers?.spirit?.map((n:number, idx:number) => <span key={idx} className="w-7 h-7 flex items-center justify-center rounded-full bg-red-900/30 text-[10px] font-black text-red-500 border border-red-900/50">{n}</span>)}
                  </div>
                </div>
              ))}
            </div>
                        ...
                        <div key={i} className={`bg-zinc-900 border rounded-[2rem] p-7 transition-all ${isWinner ? 'border-yellow-500 shadow-lg' : 'border-zinc-800 opacity-60'}`}>
                            <div className="flex justify-between items-center mb-6">
                                <div className="text-left">
                                  <span className="text-[11px] text-zinc-600 font-black block mb-1">{new Date(t.createdAt).toLocaleDateString()}</span>
                                  <span className={`text-[9px] font-black uppercase px-3 py-1 rounded-full ${isWinner ? 'bg-yellow-500 text-black animate-pulse' : isClaimed ? 'bg-zinc-700 text-zinc-300' : 'text-zinc-500'}`}>
                                    {isWinner ? "Winner (Waiting)" : isClaimed ? "Claim Requested" : "Finished"}
                                  </span>
                                </div>
                                {isWinner && (
                                  <button onClick={() => handleClaimPrize(t)} className="bg-yellow-500 text-black font-black text-[10px] px-4 py-2 rounded-xl active:scale-95 transition-transform">🎁 CLAIM</button>
                                )}
                            </div>
      ...
      <div className="w-full max-w-md flex justify-center pb-10 mt-10">
