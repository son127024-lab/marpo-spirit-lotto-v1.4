...
const uiText: Record<string, any> = {
  en: { pool: "MAR-Ω Reward Matching", reveal: "Insider Reveal", vault: "Vault Authorized network", board: "Tactical Board", submit: "Submit Draw", adMine: "Watch Ad to Unlock Board" },
  ko: { pool: "MAR-Ω 보상 매칭", reveal: "인사이더 공개", vault: "인증된 금고 상태", board: "전술 보드", submit: "번호 제출", adMine: "광고 시청 후 보드 잠금해제" }
};
    ...
    <div className="min-h-screen bg-[#050505] text-white p-6 pb-48 flex flex-col items-center font-sans relative overflow-hidden">
      ...
      <div className="w-full max-w-md mt-4 mb-8 flex items-start gap-6 relative group z-20">
        ...
        <div className="relative flex-1 bg-zinc-900/60 border border-zinc-800 rounded-[2.5rem] p-7 shadow-2xl backdrop-blur-xl">
          ...
          <p className="text-base font-bold text-zinc-300 leading-relaxed italic px-2">"{rabbit.msg}"</p>
        </div>
      </div>
      ...
      <header className="w-full max-w-md flex justify-between items-center py-6 z-10">
        <div className="bg-zinc-900/50 px-6 py-4 rounded-2xl border border-[#f39c12]/30 shadow-xl text-center">
          <p className="text-[11px] text-zinc-600 font-black mb-1 uppercase tracking-widest italic">Management</p>
          <p className="text-[#f39c12] font-black text-base italic uppercase tracking-tighter">K1 Partner</p>
        </div>
        <div className="bg-zinc-900/50 px-6 py-4 rounded-2xl border border-emerald-500/30 text-right">
          <p className="text-[11px] text-zinc-600 font-black mb-1 uppercase tracking-widest">Vault Status</p>
          <p className="text-white font-black text-base uppercase">Authorized network</p>
        </div>
      </header>

      {/* 리워드 풀 */}
      <section className="w-full max-w-md bg-gradient-to-br from-[#1a1a1a] to-[#050505] p-12 rounded-[3.5rem] border border-[#f39c12]/20 mb-10 shadow-2xl text-center relative">
