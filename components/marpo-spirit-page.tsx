...
const calcFusionFeeRate = (a: number, b: number) => {
  ...
  if (highest <= 15) return 0.05;
  if (highest <= 25) return 0.07;
  if (highest <= 35) return 0.1;
  if (highest <= 44) return 0.15;
  return 0.25;
};
...
export default function MarpoSpiritPage({
  ...
  const [adCount] = useState(0);
  const [loadingProgress, setLoadingProgress] = useState(0);
  const [revealedNumber, setRevealedNumber] = useState<number | null>(null);

  const [catalysts, setCatalysts] = useState(5);
  const [masterCatalysts, setMasterCatalysts] = useState(1);
  const [dailyFusions, setDailyFusions] = useState(0);
  const [maturingItems, setMaturingItems] = useState<MaturingItem[]>([]);
  ...
  useEffect(() => {
    ...
    const textInterval = setInterval(
      () => setLineIdx((p) => (p + 1) % currentGuides.length),
      4000
    );
  ...
  const devSkipMaturation = (id: string) => {
    playMarpoSound("electric");

    setMaturingItems((prev) => {
      const next = prev.map((item) =>
        item.id === id ? { ...item, unlockAt: Date.now() - 1000 } : item
      );
  ...
  const handleReveal = () => {
    if (ohmBalance < 1000) {
      playMarpoSound("fail");
      return alert(
        lang === "ko" ? "Ω 잔액이 부족합니다." : "Insufficient MAR-Ω."
      );
    }
  ...
  const handleMining = () => {
    if (selectedNumbers.length < 6) {
      ...
      return alert(
        lang === "ko"
          ? "원소 샘플 6개를 선택해주세요!"
          : "Please select 6 element samples!"
      );
    }
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        if (matches > 0 || Math.random() > 0.7) {
          ...
          } else if (actualMatches === 2) {
            obtainedElement = Math.floor(Math.random() * 10) + 16;
          } else if (actualMatches === 3) {
            obtainedElement = Math.floor(Math.random() * 10) + 26;
          } else {
            obtainedElement = Math.floor(Math.random() * 10) + 36;
          }
          ...
          const newItem: MaturingItem = {
            ...
            num: obtainedElement,
            unlockAt: Date.now() + 12 * 60 * 60 * 1000,
          };
  ...
  const handleRetry = () => {
    ...
    if (
      userTier === "basic" ||
      (userTier === "premium" && drawCount >= 5) ||
      (userTier === "vip" && drawCount >= 10)
    ) {
  ...
  const handleSelectForFusion = (num: number) => {
    ...
    if (slotA === num) available -= 1;
    if (slotB === num) available -= 1;
  ...
  const executeFusion = () => {
    ...
    if (dailyFusions >= 3) {
      ...
      return alert(
        lang === "ko"
          ? "일일 융합 한도(3회)를 초과했습니다."
          : "Daily fusion limit (3) exceeded."
      );
    }

    if (useMasterCatalyst && masterCatalysts < 1) {
      playMarpoSound("fail");
      return alert(
        lang === "ko"
          ? "무적 융합제가 부족합니다."
          : "Not enough Master Catalysts."
      );
    }

    if (!useMasterCatalyst && catalysts < 1) {
      playMarpoSound("fail");
      return alert(
        lang === "ko"
          ? "융합제가 부족합니다. 마르포 런에서 획득하세요!"
    ...
    if (ohmBalance < cost) {
      ...
      return alert(
        lang === "ko"
          ? `융합 비용(${cost} Ω)이 부족합니다!`
          : `Not enough Ω (${cost})!`
      );
    }

    playMarpoSound("fusion");

    setOhmBalance((prev) => prev - cost);

    if (useMasterCatalyst) {
      setMasterCatalysts((prev) => prev - 1);
    } else {
      setCatalysts((prev) => prev - 1);
    }
    ...
    setInventory((prev) => {
      const next = { ...prev };
      next[slotA] = Math.max(0, (next[slotA] || 0) - 1);
      next[slotB] = Math.max(0, (next[slotB] || 0) - 1);
      return next;
    });
    ...
    const resultNum = isSuccess
      ? Math.min(45, slotA + slotB)
      : Math.min(slotA, slotB);
    ...
    const progInt = setInterval(
      () => setLoadingProgress((p) => (p >= 100 ? 100 : p + 1)),
      40
    );
    ...
    setTimeout(() => {
      ...
      if (isSuccess) {
        playMarpoSound("success");
        setGameState("fusion_success");
      } else {
        playMarpoSound("fail");
        setGameState("fusion_fail");
      }
    ...
    <div className="min-h-screen bg-[#050505] text-white p-6 pb-48 flex flex-col items-center font-sans relative overflow-x-hidden">
      ...
      {gameState === "idle" && (
        <div className="w-full max-w-md flex justify-between items-center mb-4 relative z-20 mt-4">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-full bg-purple-900 border border-purple-500 flex items-center justify-center text-white font-bold text-xs">
              Pi
            </div>
          ...
          {!piUser && (
            <button
              ...
              className="px-3 py-1.5 bg-purple-600 hover:bg-purple-500 text-white rounded-lg text-xs font-bold transition flex items-center gap-1 disabled:opacity-60"
            >
              {isAuthenticating && (
      ...
      {gameState === "mining_video" && (
        <div className="fixed inset-0 z-[1000] bg-black flex flex-col items-center justify-center">
          ...
          <div className="relative z-10 flex flex-col items-center gap-6">
            ...
            <p className="text-4xl font-black text-amber-500 uppercase tracking-[0.4em]">
              Mining...
            </p>
          </div>
        </div>
      ...
      {gameState === "analyzing" && (
        <div className="fixed inset-0 z-[1100] bg-black flex flex-col items-center justify-center overflow-hidden">
          ...
          <div className="relative z-10 w-full max-w-md flex flex-col items-center gap-10 px-10">
            <p className="text-3xl font-black text-amber-500 uppercase tracking-widest animate-infinite-blink [text-shadow:0_2px_10px_rgba(0,0,0,1)]">
              Mar원소 분석 중.....
            </p>
      ...
      {gameState === "win_result" && (
        <div className="fixed inset-0 z-[1200] bg-black/95 flex flex-col items-center justify-center p-6 animate-in zoom-in duration-300">
          <div className="relative z-10 text-center flex flex-col items-center">
            <div className="relative w-64 h-64 mb-4 animate-bounce-in">
              <Image
                src="/토끼원석.png"
                alt="Winner Rabbit"
                fill
            ...
            <h2 className="text-6xl font-black text-amber-500 mb-2 italic uppercase tracking-tighter">
              SUCCESS!
            </h2>

            <p className="text-4xl font-black text-white mb-6">
              +{wonAmount.toLocaleString()} Ω
            </p>
              ...
              <div className="flex flex-col items-center gap-2 bg-zinc-900/90 px-8 py-5 rounded-[2rem] border-2 border-lime-500 shadow-[0_0_20px_rgba(132,204,22,0.4)] animate-in slide-in-from-bottom duration-500 delay-300">
                <div className="flex items-center gap-4">
                  <div className="relative w-9 h-9">
                    <Image
                      src={getElementIcon(earnedElement)}
                      alt="Element"
                      fill
                      className="object-contain"
                      unoptimized
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-xl font-black text-white">
                      {lang === "ko"
                        ? `${earnedElement}번 원석 발견!`
                        : `Found #${earnedElement}!`}
                    </p>
                  </div>
                </div>

                <p className="text-xs text-zinc-400 mt-2 italic bg-black/50 px-3 py-1 rounded-full">
                  <Timer size={12} className="inline mr-1" /> 12시간 후 융합
                  가능
                </p>
              </div>
            )}

            <div className="firework-container absolute inset-0 -z-10 overflow-hidden">
              {[...Array(10)].map((_, i) => (
                <div
                  key={i}
                  className="firework"
      ...
      {gameState === "fusion_analyzing" && (
        <div className="fixed inset-0 z-[2600] bg-black flex flex-col items-center justify-center overflow-hidden">
          <div className="absolute inset-0 w-full h-full z-0 opacity-40 mix-blend-color-burn bg-red-900" />
          ...
          <div className="absolute inset-0 w-full h-full z-0">
            <Image
              ...
              alt="Hourglass"
              fill
              className="object-cover opacity-80 hue-rotate-180 brightness-150 contrast-150"
              priority
              unoptimized
            />
          </div>
          ...
          <div className="relative z-10 w-full max-w-md flex flex-col items-center gap-10 px-10">
            <p className="text-4xl font-black text-red-500 uppercase tracking-widest animate-infinite-blink [text-shadow:0_0_20px_rgba(255,0,0,1)]">
              FUSING ELEMENTS...
            </p>
      ...
      {gameState === "fusion_success" && fusionResultNode && (
        <div className="fixed inset-0 z-[2600] bg-black/95 flex flex-col items-center justify-center p-6 animate-in zoom-in duration-300">
          <div className="relative z-10 text-center flex flex-col items-center">
            <h2 className="text-6xl font-black text-red-500 mb-6 italic uppercase tracking-tighter drop-shadow-[0_0_30px_rgba(255,0,0,0.8)]">
              SUCCESS!
            </h2>

            <div className="relative w-48 h-48 mb-8 animate-bounce-in shadow-[0_0_100px_rgba(255,0,0,0.5)] rounded-full">
              <Image
                src={getElementIcon(fusionResultNode)}
                alt="Fused Element"
                fill
                className="object-contain"
                unoptimized
            ...
            <p className="text-3xl font-black text-white bg-red-900/50 px-8 py-4 rounded-full border border-red-500/50">
              {lang === "ko"
                ? `${fusionResultNode}번 원석으로 진화했습니다!`
                : `Evolved into #${fusionResultNode}!`}
            </p>
      ...
      {gameState === "fusion_fail" && fusionResultNode && (
        <div className="fixed inset-0 z-[2600] bg-black/95 flex flex-col items-center justify-center p-6 animate-in fade-in duration-500 overflow-hidden">
          <div className="fixed inset-0 z-0 flex items-center justify-center pointer-events-none">
            {renderCoalParticles(true)}
          </div>

          <div className="relative z-10 flex flex-col items-center">
            <h2 className="text-5xl font-black text-zinc-600 mb-8 italic uppercase tracking-tighter line-through">
              FAILED
            </h2>
            ...
            <p className="text-xl font-bold text-zinc-400 bg-zinc-900/80 px-8 py-4 rounded-2xl text-center break-keep border border-zinc-800">
              {lang === "ko"
                ? `붕괴되어 ${fusionResultNode}번 원석만 남았습니다.`
                : `Collapsed. Only #${fusionResultNode} remained.`}
            </p>
          </div>
        </div>
      )}
        ...
        <>
          <div className="w-full max-w-md mb-6 flex items-start gap-6 relative z-20">
            ...
            <div className="relative flex-1 bg-zinc-900/70 border border-zinc-800 rounded-[2.5rem] p-7 shadow-2xl backdrop-blur-xl min-h-[120px] flex items-center">
              <div className="absolute top-10 -left-4 w-0 h-0 border-t-[12px] border-t-transparent border-r-[20px] border-r-zinc-800 border-b-[12px] border-b-transparent" />
              <p className="text-[16px] md:text-[18px] font-bold text-zinc-200 italic px-2">
                &quot;{currentGuides[lineIdx]}&quot;
              </p>
            </div>
          </div>
          ...
          <div className="w-full max-w-md grid grid-cols-3 gap-2 mb-8 relative z-20">
            <div className="col-span-3 bg-gradient-to-br from-[#1a1a1a] to-[#050505] p-6 rounded-[2.5rem] border border-[#f39c12]/20 flex flex-col items-center justify-center shadow-lg">
              <p className="text-[10px] text-zinc-500 font-black uppercase tracking-[0.3em] mb-1">
                Ω Energy Balance
              </p>

              <div className="flex items-center gap-2">
                <p className="text-4xl font-black text-white font-mono tracking-tighter">
                  {ohmBalance.toLocaleString()}
                </p>
                <span className="text-amber-500 text-2xl font-black italic">
                  Ω
                </span>
              </div>
            </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[10px] text-zinc-400 uppercase font-black tracking-widest">
                    {lang === "ko" ? "무적 융합제" : "Master Catalyst"}
                  </p>
          ...
          <div className="w-full max-w-md flex flex-col gap-3 mb-10 relative z-20">
            <button
              ...
              className="w-full py-5 bg-zinc-900/40 border border-[#f39c12]/40 rounded-3xl flex flex-col items-center active:scale-95 transition-all"
            >
              <div className="flex items-center gap-2 mb-1">
                <Target size={18} className="text-[#f39c12]" />
                <p className="text-[#f39c12] font-black text-xs uppercase tracking-widest">
                  Insider Reveal
                </p>
              </div>
            ...
            <button
              ...
              className="w-full py-6 bg-gradient-to-r from-amber-600 to-amber-500 rounded-3xl flex items-center justify-center gap-4 shadow-[0_10px_20px_rgba(0,0,0,0.3)] active:scale-95 transition-all border-b-4 border-amber-800"
            >
                ...
                return (
                  <button
                    ...
                    className={`relative aspect-square rounded-xl overflow-hidden transition-all duration-300 transform ${
                      isSelected
                        ? "border-2 border-amber-500 ring-2 ring-amber-500 z-10 bg-amber-500/20"
                        : isHint
                          ? "border-2 border-lime-500 animate-pulse scale-105 z-10 bg-lime-500/20"
                          : "border border-zinc-800 bg-black"
                    }`}
                  >
                    <div
                      className={`absolute inset-0 bg-cover bg-center transition-opacity ${
                        isSelected || isHint ? "opacity-30" : "opacity-100"
                      }`}
                      style={{
                        backgroundImage: `url('${getElementIcon(num)}')`,
                      }}
                    />

                    <div
                      className={`absolute inset-0 flex items-center justify-center ${
                        isSelected || isHint ? "opacity-100" : "opacity-0"
                      }`}
                    >
                      <span
                        className={`text-2xl font-black ${
                          isSelected ? "text-amber-500" : "text-lime-500"
                        }`}
                      >
                        {num}
                      </span>
          ...
          <div className="w-full max-w-md bg-zinc-900/50 p-6 rounded-[3.5rem] border border-zinc-800 flex justify-center items-center shadow-2xl z-20 mb-10">
            <button
              ...
              className="w-full py-5 rounded-3xl bg-amber-500 text-black font-black text-lg uppercase shadow-[0_0_20px_rgba(243,156,18,0.4)] active:scale-95 transition-all flex justify-center items-center gap-2 disabled:opacity-50"
            >
              <Pickaxe size={24} />
              {lang === "ko" ? "채굴 시작 (MINING)" : "START MINING"}
            </button>
          </div>
        </>
      )}
      ...
      {showVault && gameState === "idle" && (
        <div
          className={`fixed inset-0 h-dvh z-[2500] ${
            isFusionMode ? "bg-[#0a0000]" : "bg-black"
          } animate-in slide-in-from-bottom duration-500 flex flex-col px-6 pt-6 pb-0 overflow-y-auto overscroll-contain touch-pan-y transition-colors`}
        >
          <div className="flex justify-between items-center mb-8 mt-6">
            <div>
              <h2
                className={`text-4xl font-black italic tracking-tighter uppercase transition-colors ${
                  isFusionMode ? "text-red-500" : "text-amber-500"
                }`}
              >
          ...
          {!isFusionMode && maturingItems.length > 0 && (
            <div className="mb-10 w-full bg-zinc-900/40 p-5 rounded-[2rem] border border-cyan-900/50">
              <h3 className="text-cyan-400 font-black italic uppercase tracking-widest text-sm mb-4 flex items-center gap-2">
                <Timer size={16} />
                Incubator (Maturing)
              </h3>
                ...
                {maturingItems.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() =>
                        isReady
                          ? harvestElement(item.id, item.num)
                          : devSkipMaturation(item.id)
                      }
                      className={`relative shrink-0 w-24 h-24 rounded-2xl border-2 transition-all snap-start ${
                        isReady
                          ? "border-lime-500 animate-pulse"
                          : "border-zinc-700"
                      }`}
                    >
                      <Image
                        src={getElementIcon(item.num)}
                        alt="Maturing"
                        fill
                        className={`object-contain p-2 ${
                          isReady ? "" : "grayscale opacity-30"
                        }`}
                        unoptimized
                      />

                      <div
                        className={`absolute inset-0 flex flex-col items-center justify-center rounded-2xl bg-black/60 backdrop-blur-[2px] ${
                          isReady ? "opacity-0 hover:opacity-100" : ""
                        }`}
                      >
                        {isReady ? (
                          <div className="bg-lime-500 text-black text-xs font-black px-3 py-1 rounded-full flex items-center gap-1">
                            <CheckCircle2 size={12} />
                            CLAIM
                          </div>
                        ) : (
                          <span className="text-cyan-300 font-mono text-sm font-black drop-shadow-md">
                            {formatTimeLeft(item.unlockAt)}
                          </span>
                        )}
                      </div>
                    </button>
          ...
          {isFusionMode && (
            <div className="w-full relative mt-2 mb-10 shrink-0">
              <div className="absolute inset-0 bg-gradient-to-b from-[#1a0505] to-black border border-red-900/50 rounded-[2.5rem] shadow-[0_0_40px_rgba(220,38,38,0.15)] pointer-events-none" />

              <div className="relative z-10 flex flex-col items-center p-6 pt-10">
                <div className="absolute top-[30%] left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 h-48 bg-red-600/20 blur-[80px] pointer-events-none rounded-full" />

                <div className="flex items-center justify-center gap-4 sm:gap-6 mb-10 w-full relative">
                  <div className="flex flex-col items-center gap-3">
                    <button
                      onClick={() => handleRemoveFromSlot("A")}
                      className={`relative w-28 h-28 sm:w-32 sm:h-32 rounded-[2rem] flex items-center justify-center transition-all duration-300 ${
                        slotA
                          ? "bg-gradient-to-br from-zinc-900 to-black border-2 border-red-500 shadow-[0_0_30px_rgba(239,68,68,0.4)] scale-105"
                          : "bg-black/50 border-2 border-dashed border-zinc-700/80 hover:border-zinc-500"
                      }`}
                    >
                      ...
                      {slotA ? (
                        <Image
                          src={getElementIcon(slotA)}
                          alt="A"
                          width={85}
                          height={85}
                          unoptimized
                          className="animate-pulse drop-shadow-[0_0_25px_rgba(255,255,255,0.4)]"
                        />
                      ) : (
                    ...
                    <div className="bg-black/80 px-4 py-1.5 rounded-full border border-red-900/50 shadow-lg">
                  ...
                  <div className="flex items-center justify-center w-10 h-10 rounded-full bg-black border border-red-600/50 shadow-[0_0_15px_rgba(239,68,68,0.5)] z-10">
                    <Plus size={20} className="text-red-500" strokeWidth={4} />
                  </div>

                  <div className="flex flex-col items-center gap-3">
                    <button
                      onClick={() => handleRemoveFromSlot("B")}
                      className={`relative w-28 h-28 sm:w-32 sm:h-32 rounded-[2rem] flex items-center justify-center transition-all duration-300 ${
                        slotB
                          ? "bg-gradient-to-br from-zinc-900 to-black border-2 border-red-500 shadow-[0_0_30px_rgba(239,68,68,0.4)] scale-105"
                          : "bg-black/50 border-2 border-dashed border-zinc-700/80 hover:border-zinc-500"
                ...
                <div className="w-full max-w-sm flex justify-between items-center bg-white/5 backdrop-blur-md p-4 rounded-2xl mb-6 border border-white/10 z-10">
                  <div className="flex items-center gap-3">
                    <Gem
                      size={22}
                      className={
                        masterCatalysts > 0 ? "text-fuchsia-400" : "text-zinc-600"
                      }
                    />
                  ...
                  <button
                    onClick={() => {
                      playMarpoSound("click");
                      setUseMasterCatalyst(!useMasterCatalyst);
                    }}
                    disabled={masterCatalysts < 1}
                    className={`w-14 h-8 rounded-full flex items-center transition-all px-1 ${
                      useMasterCatalyst
                        ? "bg-fuchsia-600 justify-end shadow-[0_0_15px_rgba(192,38,211,0.5)]"
                ...
                <div className="w-full max-w-sm bg-black/60 rounded-2xl p-5 flex justify-between items-center mb-8 z-10 border border-zinc-800 shadow-inner">
                  ...
                  <div className="text-right flex flex-col">
                    ...
                    <p
                      className={`text-3xl font-black italic ${
                        slotA && slotB ? "text-red-500" : "text-zinc-800"
                      }`}
                    >
                ...
                <button
                  ...
                  disabled={!slotA || !slotB || dailyFusions >= 3}
                  className={`w-full max-w-sm py-5 rounded-2xl font-black text-2xl tracking-[0.2em] uppercase transition-all flex justify-center items-center gap-3 z-10 ${
                    slotA && slotB && dailyFusions < 3
                      ? useMasterCatalyst
                        ? "bg-fuchsia-600 shadow-[0_0_30px_rgba(192,38,211,0.6)] text-white"
                ...
                >
                  {useMasterCatalyst ? (
                    <>
                      <Gem size={24} /> MASTER FUSE
                    </>
                  ) : (
                    <>
                      <Zap size={24} /> FUSE ELEMENTS
                    </>
                  )}
                </button>
              </div>
          ...
          <div className="mt-8 mb-4 flex justify-between items-end relative z-10 shrink-0">
            <h3
              className={`font-black italic uppercase tracking-widest text-sm ${
                isFusionMode ? "text-red-500" : "text-amber-500"
              }`}
            >
              {isFusionMode ? "Select Ingredients" : "Stored Raw Materials"}
            </h3>
          ...
          <div className="grid grid-cols-5 gap-3 mb-32 relative z-10 shrink-0">
            {[...Array(45)].map((_, i) => {
              ...
              if (isFusionMode && slotA === num) count -= 1;
              if (isFusionMode && slotB === num) count -= 1;
              ...
              return (
                ...
                >
                  <div
                    className={`absolute inset-0 bg-cover bg-center rounded-2xl border-2 ${
                      isAvailable
                        ? isFusionMode
                  ...
                  {isAvailable && (
                    <div
                      className={`absolute -top-2 -right-2 ${
                        isFusionMode ? "bg-red-500" : "bg-amber-500"
                      } text-black text-[9px] font-black px-1.5 py-0.5 rounded-full shadow-lg z-20`}
                    >
                      {count}
                    </div>
                  )}

                  <div className="absolute bottom-1 left-1/2 -translate-x-1/2 text-[10px] font-black text-white/80 drop-shadow-md z-10">
                    {num}
                  </div>
                </button>
              );
          ...
          <div className="mt-auto flex gap-4">
            {isFusionMode ? (
              <button
                ...
                className="w-full py-5 bg-zinc-800 text-white rounded-2xl font-black text-lg uppercase active:scale-95 transition-transform"
              >
              ...
              <>
                <button
                  ...
                  className="flex-1 py-5 bg-zinc-800 text-white rounded-2xl font-black text-sm uppercase active:scale-95 transition-transform"
                >
                  {lang === "ko" ? "채굴장 복귀" : "Mine"}
                </button>
                ...
                <button
                  ...
                  className="flex-[2] py-5 bg-red-600 text-white rounded-2xl font-black text-lg uppercase flex justify-center items-center gap-2 shadow-[0_0_20px_rgba(220,38,38,0.4)] active:scale-95 transition-transform"
                >
                  <Zap size={20} />
                  {lang === "ko" ? "융합소 입장" : "Enter Fusion Lab"}
                </button>
              </>
            )}
          </div>
      ...
      <style jsx global>{`
        @keyframes coalExplode {
          ...
          100% {
            transform: scale(1.5) translate(var(--dir-x), var(--dir-y))
              rotate(var(--rot));
            opacity: 0;
          }
        }
        ...
        .coal-particle {
          ...
          animation: coalExplode 2s cubic-bezier(0.1, 1, 0.3, 1)
            var(--delay) forwards;
        }
