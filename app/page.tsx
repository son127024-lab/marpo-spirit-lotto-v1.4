...
import Link from 'next/link'; // 🚩 [에러수정] next/image가 아닌 next/link에서 가져와야 합니다.
import WinnerBoard from '../components/WinnerBoard';

// 🏆 당첨 리포트 컴포넌트 (속성명 first, second, third로 수정하여 에러 방지)
const WinningReport = () => {
  const winners = [
        ...
        {winners.map((w, i) => (
          <div key={i} className="bg-zinc-900/80 border border-zinc-800 rounded-[2.5rem] p-8 shadow-2xl relative">
            ...
            <div className="grid grid-cols-3 gap-3 border-t border-zinc-800/50 pt-5">
              <div>
                <p className="text-[9px] text-zinc-600 font-black uppercase mb-1">1st Rank</p>
                <p className="text-sm font-black text-white">{w.first} <span className="text-[10px] text-zinc-600">π</span></p>
              </div>
              <div>
                <p className="text-[9px] text-zinc-600 font-black uppercase mb-1">2nd Rank</p>
                <p className="text-sm font-black text-white">{w.second} <span className="text-[10px] text-zinc-600">π</span></p>
              </div>
              <div>
                <p className="text-[9px] text-zinc-600 font-black uppercase mb-1">3rd Rank</p>
                <p className="text-sm font-black text-white">{w.third} <span className="text-[10px] text-zinc-600">π</span></p>
              </div>
            </div>
          </div>
        ))}
...
export default function MarpoLottoPage() {
  ...
  const fetchOracleSettings = useCallback(async () => {
    ...
    } catch (e) { console.error("Oracle Background Sync Fail"); }
  }, []);
  ...
  const fetchMyTickets = useCallback(async (userId: string) => {
    ...
    } catch (error) { console.error("Ticket data error"); }
  }, []);
    ...
    <div className="min-h-screen bg-black text-white flex flex-col items-center p-4 font-sans relative pb-40 text-center">
      ...
      <section className="w-full max-w-md bg-zinc-900 border border-yellow-500/20 p-8 rounded-[2.5rem] mb-12 shadow-2xl relative overflow-hidden">
          ...
          <p className="text-5xl font-black text-white tracking-tighter mb-8">
            {jackpot.toLocaleString(undefined, {minimumFractionDigits: 4})} <span className="text-xl text-zinc-600 font-normal">π</span>
          </p>
          <div className="pt-6 border-t border-zinc-800/80 flex justify-between items-end px-1">
            ...
            <div className="text-right">
              <p className="text-xl font-black text-white tracking-tight leading-none">
                ...
                <span className="text-[10px] text-zinc-500 ml-1.5 font-bold uppercase">usd</span>
              </p>
            </div>
          </div>
      </section>
              ...
              {showHistory && (
                <div className="mt-8 flex flex-col gap-6">
                  {historyTickets.map((t, i) => {
                    ...
                    return (
                        <div key={i} className={`bg-zinc-900 border rounded-[2rem] p-8 ${isWon ? 'border-yellow-500' : 'border-zinc-800 opacity-60'}`}>
