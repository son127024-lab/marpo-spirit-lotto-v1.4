...
// 🚨 경로 수정: 컴포넌트 폴더가 app 폴더 밖에 있다면 '../components/WinnerBoard' 가 맞습니다.
...
export default function MarpoLottoPage() {
  ...
  const saveTicketToDB = async (txid: string) => {
    try {
      const res = await fetch('/api/tickets', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ numbers: { main: mainNumbers, spirit: spiritNumbers }, userId: user?.username, amount: ticketPrice, transactionId: txid }) });
      if (res.ok) { 
    ...
    try {
      ...
      if (myData.success && myData.tickets.length > 0) {
        ...
        if (latestTicket.status === "COMPLETED") {
          alert(`Your ticket is sealed, @${user?.username}.\nWait for Draw (Fri 20:00)! 🏎️💨`);
        } else if (latestTicket.status === "WON" || latestTicket.status === "CLAIMED") {
          try {
            const audio = new Audio('/win.mp3');
            audio.play().catch(() => {});
          } catch (e) {}
          confetti({ particleCount: 200, spread: 120, origin: { y: 0.6 }, colors: ['#EAB308', '#DC2626', '#FFFFFF'], zIndex: 9999 });
          setTimeout(() => { alert(`🎉 Congratulations, @${user?.username}!\nYou won the ${latestTicket.rank} prize! 🏆`); }, 500);
        } else {
          alert(`Better luck next time, @${user?.username}!`);
        }
      } else {
        alert("No tickets found. Play first!");
      }
    } catch (e) {
      alert("Network error.");
    } finally {
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col items-center p-4 font-sans relative pb-20">
      ...
      <div className="w-full max-w-md flex justify-between items-start pt-6 mb-6 px-2 relative mt-4">
        <div className="flex flex-col items-start">
          ...
          <div className="mt-2 h-6">
            {user ? <p className="text-yellow-500 font-black text-xs tracking-widest uppercase">@{user.username}</p> : <p className="text-zinc-600 text-[10px] tracking-[0.3em] uppercase animate-pulse">Connecting...</p>}
          </div>
        </div>
        ...
        <div className="bg-zinc-900/80 border border-zinc-800 rounded-xl px-4 py-2 flex flex-col items-end shadow-lg backdrop-blur-sm">
          <p className="text-[8px] text-zinc-500 uppercase tracking-widest font-black mb-1">Global Pi Price</p>
          <div className="flex items-center gap-2">
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span>
              <span className="relative inline-flex rounded-full h-2 w-2 bg-green-500"></span>
            </span>
            <p className="text-sm font-black text-white tracking-wider">$ {peggedUsd} <span className="text-zinc-500 text-[10px]">USD</span></p>
          </div>
        </div>
      </div>
      ...
      {/* 번호 선택 영역 생략 (기본 로직 유지) */}
      <section className="w-full max-w-md bg-zinc-900/50 p-6 rounded-3xl border border-zinc-800 mb-6 text-center">
        <p className="text-xs text-zinc-400 uppercase tracking-widest mb-4">Pick Your Numbers</p>
        <div className="grid grid-cols-7 gap-2 mb-10">
          {Array.from({ length: 45 }, (_, i) => i + 1).map((n) => (
            <button key={`m-${n}`} onClick={() => toggleMainNumber(n)} className={`h-11 w-11 rounded-full text-xs font-black transition-all ${mainNumbers.includes(n) ? 'bg-yellow-500 text-black scale-110 shadow-lg' : 'bg-zinc-800 text-zinc-500'}`}>{n}</button>
          ))}
        </div>
        <div className="grid grid-cols-7 gap-2">
          {Array.from({ length: 45 }, (_, i) => i + 1).map((n) => (
            <button key={`s-${n}`} onClick={() => toggleSpiritNumber(n)} className={`h-11 w-11 rounded-full text-xs font-black transition-all ${spiritNumbers.includes(n) ? 'bg-red-600 text-white scale-110 shadow-lg' : 'bg-zinc-800 text-zinc-500'}`}>{n}</button>
          ))}
        </div>
      </section>

      <button onClick={() => setIsModalOpen(true)} disabled={mainNumbers.length !== 8 || spiritNumbers.length !== 2 || !user} className="w-full max-w-md py-5 rounded-2xl font-black text-2xl tracking-[0.3em] mb-16 bg-gradient-to-r from-yellow-600 to-yellow-500 text-black shadow-xl disabled:opacity-50 transition-all">
        PLAY {ticketPrice} PI
      </button>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-sm flex justify-center items-center z-50 p-6">
          <div className="bg-zinc-900 border-2 border-yellow-500/50 p-8 rounded-[2.5rem] w-full max-w-md relative shadow-2xl text-center">
            ...
            <div className="bg-zinc-800/50 border border-zinc-800 rounded-2xl p-6 mb-8 mt-6">
            ...
            <button onClick={handlePaymentSubmit} disabled={isStoring} className="w-full bg-gradient-to-r from-yellow-500 to-yellow-400 text-black font-black text-xl py-5 rounded-2xl uppercase tracking-widest shadow-lg">{isStoring ? 'STORING...' : `Pay ${ticketPrice} Pi`}</button>
          </div>
        </div>
      )}
    </div>
