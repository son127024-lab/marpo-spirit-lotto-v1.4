  ...
  interface Window {
    Pi?: {
      ...
      Ads?: {
        ...
        showRewardedVideo?: () => Promise<{ adFinished: boolean }>;
      };
    };
  }
}
...
export default function MainGameLobby() {
  ...
  const verifyPiAccessTokenWithBackend = async (accessToken: string): Promise<PiUser> => {
    ...
    const data = (await response.json()) as PiAuthResponse | PiAuthErrorResponse;

    if (!response.ok || !('success' in data)) {
      throw new Error('error' in data ? data.error : 'Pi backend verification failed.');
    }

    return data.user;
  };

  const authenticatePi = useCallback(
    async (source: 'auto' | 'manual' = 'manual') => {
      if (typeof window === 'undefined' || !window.Pi) {
        const message = lang === 'ko' ? 'Pi SDK가 아직 로드되지 않았습니다.' : 'Pi SDK is not loaded yet.';
        setAuthError(message);
        if (source === 'manual') alert(message);
        return;
      }
      ...
      try {
        ...
        const verifiedUser = await verifyPiAccessTokenWithBackend(authResult.accessToken);
        setPiUser(verifiedUser);

        if (Pi.Ads?.preloadRewardedVideo) {
          await Promise.resolve(Pi.Ads.preloadRewardedVideo());
        }

        if (source === 'manual' && agreed) {
        ...
        setAuthError(message);
        if (source === 'manual') {
          alert(lang === 'ko' ? `Pi 인증에 실패했습니다. ${message}` : `Pi authentication failed. ${message}`);
        }
      } finally {
        setIsAuthenticating(false);
      }
    },
    [agreed, isAuthenticating, lang]
  );

  const triggerAutoAuth = useCallback(() => {
    if (autoAuthStartedRef.current || piUser) return;
    autoAuthStartedRef.current = true;
    void authenticatePi('auto');
  }, [authenticatePi, piUser]);

  useEffect(() => {
    if (!isReady || piUser || autoAuthStartedRef.current) return;
  ...
  }, [isReady, piUser, triggerAutoAuth]);
  ...
  const t = {
    ko: {
      ...
      desc1: '단순한 일회성 결제를 넘어, SaaS(Software as a Service) 기반의 지속 가능한 구독 모델을 통해 Pi의 상시 유틸리티를 창출합니다. 이는 생태계 내에 고여있는 토큰을 끊임없이 순환시키고 가치를 재분배하는 강력한 혈관 역할을 수행합니다.',
      desc2: "우리는 파이오니어들의 장기적인 참여와 기여를 이끌어내는 고유의 전술적 운영 프로토콜을 통해, 공급과 수요의 균형을 맞추고 Pi의 거시적 희소성을 보호하는 '디플레이션 선순환' 체계를 구축하고자 합니다.",
    ...
    en: {
      warnSub: 'This application is a demo version for Pi Core Team and ecosystem validation. Real Pi coins will not be deducted.',
      bold1: 'Marpo Group ',
      bold2: 'completes the circular economy of the Pi ecosystem by utilizing this Web3 SaaS system.',
      desc1: 'Beyond simple one-off payments, we create constant utility for Pi through a sustainable SaaS (Software as a Service) based subscription model. This acts as a powerful vascular system, continuously circulating stagnant tokens within the ecosystem and redistributing value.',
      desc2: "Through our unique tactical operational protocol that drives long-term participation and contribution from Pioneers, we aim to build a 'deflationary virtuous cycle' system that balances supply and demand and protects the macroeconomic scarcity of Pi.",
      langBtn: '한국어로 보기',
  ...
  return (
    <>
      <Script src="https://sdk.minepi.com/pi-sdk.js" strategy="afterInteractive" onLoad={triggerAutoAuth} />
          ...
          <div className="relative z-10 w-full max-w-4xl flex flex-col items-center gap-6 bg-black/60 p-10 rounded-[3rem] border border-zinc-800 backdrop-blur-md shadow-2xl mt-10">
            ...
            <div className="w-full bg-zinc-900/80 p-6 md:p-8 rounded-3xl border border-zinc-700 text-left mt-2">
              ...
              <p className="text-zinc-200 text-sm md:text-lg leading-relaxed mb-6 font-bold">
                {t[lang].bold1} <span className="text-amber-400">{t[lang].bold2}</span>
              </p>
            ...
            <button onClick={() => setLang(lang === 'ko' ? 'en' : 'ko')} className="px-5 py-2.5 rounded-full border border-zinc-700 bg-zinc-800/80 text-zinc-300 text-xs font-bold uppercase">
            ...
            {piUser ? (
              ...
              <p className="text-xs font-bold text-red-400 mt-2 text-center">{authError}</p>
            ) : (
              <p className="text-xs font-bold text-zinc-500 mt-2">{isAuthenticating ? 'Waiting for Pi sign-in...' : 'Pi sign-in will start automatically.'}</p>
            )}

            <div className="flex flex-col md:flex-row gap-3">
              <button
                onClick={() => authenticatePi('manual')}
        ...
        <div className="min-h-screen w-full bg-[#050505] text-white flex flex-col items-center justify-center p-6 relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-6xl">
            <div className="bg-zinc-900 border border-zinc-700 rounded-[3.5rem] p-8 flex flex-col items-center">
              ...
              <button onClick={() => handlePayment('basic')} className="w-full py-4 bg-zinc-800 text-white rounded-xl font-black uppercase text-xs">
            ...
            <div className="bg-zinc-900 border-2 border-amber-500 rounded-[3.5rem] p-8 flex flex-col items-center transform md:-translate-y-4">
              ...
              <button onClick={() => handlePayment('premium')} className="w-full py-5 bg-amber-500 text-black rounded-xl font-black uppercase text-sm">
                Mock Pi Payment
              </button>
            </div>
            <div className="bg-zinc-900 border border-lime-500/50 rounded-[3.5rem] p-8 flex flex-col items-center">
              ...
              <button onClick={() => handlePayment('vip')} className="w-full py-4 bg-lime-500 text-black rounded-xl font-black uppercase text-xs">
                Mock Pi Payment
              </button>
            </div>
          </div>
          <button onClick={() => setView('intro')} className="mt-12 text-zinc-500 text-[10px] font-bold underline">
            {t[lang].cancelBtn}
          </button>
        </div>
      )}
        ...
        <div className="min-h-screen w-full bg-[#050505] text-white relative flex flex-col items-center">
          <header className="w-full p-4 md:px-10 flex justify-between items-center z-50 bg-black/50 backdrop-blur-md sticky top-0 border-b border-zinc-800">
            ...
            <div className="flex items-center gap-4">
              <div className="text-[10px] font-bold text-amber-400 uppercase">Commander: {piUser ? piUser.username : 'Verified'}</div>
              <button
                onClick={() => {
