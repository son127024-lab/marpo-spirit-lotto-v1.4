  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white p-6 pb-48 flex flex-col items-center font-sans relative overflow-x-hidden">
              ...
              {[...Array(45)].map((_, i) => {
                ...
                return (
                  <button key={num} onClick={() => handleNumberToggle(num)} className={`relative aspect-square rounded-xl overflow-hidden transition-all duration-300 transform ${isSelected ? 'border-2 border-amber-500 ring-2 ring-amber-500 z-10 bg-amber-500/20' : isHint ? 'border-2 border-lime-500 animate-pulse scale-105 z-10 bg-lime-500/20' : 'border border-zinc-800 bg-black'}`}>
                    ...
                    <div className={`absolute inset-0 flex items-center justify-center ${isSelected || isHint ? 'opacity-100' : 'opacity-0'}`}><span className={`text-2xl font-black ${isSelected ? 'text-amber-500' : 'text-lime-500'}`}>{num}</span></div>
                  </button>
                );
              })}
            </div>
      ...
      {showVault && gameState === 'idle' && (
        <div className={`fixed inset-0 h-dvh z-[2500] ${isFusionMode ? 'bg-[#0a0000]' : 'bg-black'} animate-in slide-in-from-bottom duration-500 flex flex-col px-6 pt-6 pb-0 overflow-y-auto overscroll-contain touch-pan-y transition-colors`}>
